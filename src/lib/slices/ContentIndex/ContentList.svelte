...
<ul on:mouseleave={onMouseLeave} class="grid border-b border-b-slate-100">
	{#each items as post, index (post.id + index)}
		...
		>
			<PrismicLink
				document={post}
				class="flex flex-col justify-between border-t border-t-slate-600 py-10 text-slate-700 md:flex-row"
			>
				<div class="flex flex-col">
					<span class="text-3xl font-bold">{post.data.title}</span>
					<div class="flex gap-3 text-yellow-500">
...
<!-- Hover element -->
<div
  class="hover-reveal pointer-events-none absolute left-0 top-0 -z-10
         h-[320px] w-[220px] rounded-lg bg-cover bg-center opacity-0
         filter brightness-125 transition-[background,filter] duration-300"
